...
//build out root and views directory + index.html
fs.mkdir(fileName, function(err) {
  ...
  fs.mkdir(fileName + '/db', function(err) {
    ...
    fs.writeFile(fileName + '/db/schema.sql', 'DROP TABLE IF EXISTS', function(
      err
    ) {
      if (err) {
...
//populate app.js with express server
createServer = () => {
  fs.writeFile(
    fileName + '/app.js',
    '/////////------SET UP VARIABLES------/////////\n\
...
//import jquery script and place in public/js as jquery.js
importJquery = openEditor => {
  var options = {
    url: 'http://code.jquery.com/jquery-2.2.1.min.js',
    include: true
  };
